...
def get_data_fasttext(data_dir=DATA_DIR, save_dir= DATA_DIR+'fasttext_anno/', train_path='train.json',test_path='test.json'):
    train_list = []
    test_list = []
    ...
    with open(data_dir + train_path, 'r') as reader:
        for line in reader:
            example = json.loads(line)
            train_list.append([' '.join(segmentor.segment(example['anno_label'])),'__label__'+str(example['cls_label'])])
    with open(data_dir + test_path, 'r') as reader:
        for line in reader:
            example = json.loads(line)
            test_list.append([' '.join(segmentor.segment(example['anno_label'])),'__label__'+str(example['cls_label'])])
